...
import { getAllLibrariesAdmin, deleteLibrary, updateLibrary } from '@/features/libraries/api/libraries';
...
export default function GestionBibliotecas() {
  ...
  const handleToggleActive = async (library) => {
    ...
    if (!activating && !confirm(`¿Desactivar "${library.name}"? Dejará de mostrarse en el sitio público hasta que la reactivés.`)) return;
    try {
      await updateLibrary(library._id, { isActive: activating });
      load();
    } catch (err) {
      setError(err.response?.data?.message || 'No se pudo cambiar el estado de la biblioteca.');
    }
  };
